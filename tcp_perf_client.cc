...
int FLAGS_total_transfer = 10 * 1000 * 1000;
int FLAGS_chunk_size = 1000;
...
int main(int argc, char *argv[]) {
  // input should be like ./a.out addr duration
  int sock;
  struct sockaddr_in serv_addr;
  char buffer[256];
  std::string address(argv[1]);
  FLAGS_duration = atoi(argv[2]);
  ...
  // if (line->HasSwitch("t")) {
  //   if (!base::StringToint(line->GetSwitchValueASCII(base::StringPiece("t")), &FLAGS_total_transfer)) {
  //     cout << "-t must be an unsigned integer\n";
  //     return 1;
  //   }
  // }
  // if (line->HasSwitch("c")) {
  //   if (!base::StringToint(line->GetSwitchValueASCII("c"), &FLAGS_chunk_size)) {
  //     cout << "-c must be an unsigned integer\n";
  //     return 1;
  //   }
  // }
  // if (line->HasSwitch("d")) {
  //   if (!base::StringToint(line->GetSwitchValueASCII("d"), &FLAGS_duration)) {
  ...
  if (FLAGS_duration == 0) {
    for (int i = 0; i < FLAGS_total_transfer; i += FLAGS_chunk_size) {
